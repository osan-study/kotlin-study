...
| 기능    | 내용                                                                                |
|-------|-----------------------------------------------------------------------------------|
| 오류 보고 | 에러의 위치나 내용을 알 수 있도록 예외타입과 내용, 위치를 전달                                              |
 | 복구    | 시스템이 종료되지 않게 호출스택을 따라가며 상위처리가 되어있는지 확인 후 예외처리가 되어있을 시 시스템 복구                      |
| 자원 해제 | 특정 자원들(File, Stream 등)은 자원을 해제하지 않을 경우 메모리에 계속 유지되는데, 해당 부분을 finally 구문을 통해 처리 가능 |
...
Exception3: error
test(0): OK
test(1): Exception1: wrong value: 1
test(2): Exception2: Exception 2
test(3): Exception3: Exception 3
testCatchOrder(2): Exception2: Exception 2
testCatchOrder(3): Exception3: Exception 3
...
testFinally(0): Switch(on=false, result=exception)
testFinally(1): Switch(on=false, result=OK)
```